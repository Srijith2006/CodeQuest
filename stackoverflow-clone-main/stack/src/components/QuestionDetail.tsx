...
const QuestionDetail = ({ questionId }: any) => {
    ...
    const fetchquestion = async () => {
      try {
        ...
        const savedIds: string[] = JSON.parse(
          localStorage.getItem(user ? `saves_${(user as any)._id}` : "saves_guest") || "[]"
        );
        setquestion({ ...(matchedquestion || null), isBookmarked: matchedquestion ? savedIds.includes(matchedquestion._id) : false });
      } catch (error) {
        console.log(error);
      } finally {
        setloading(false);
  ...
  const handlebookmark = () => {
    ...
    const key = `saves_${(user as any)._id}`;
    const ids: string[] = JSON.parse(localStorage.getItem(key) || "[]");
    const isCurrentlySaved = ids.includes(question._id);
    let updated: string[];
    if (isCurrentlySaved) {
      updated = ids.filter((i: string) => i !== question._id);
      toast.info("Removed from saves");
    } else {
      updated = [...ids, question._id];
      toast.success("Saved!");
    }
  ...
  const handleSubmitanswer = async () => {
    ...
    try {
      // FIX: correct route is /answer/post/:id, and userid sent explicitly in body
      const res = await axiosInstance.post(`/answer/post/${question?._id}`, {
  ...
  const handleDeleteanswer = async (answerId: string) => {
    ...
    try {
      // FIX: correct route is /answer/delete/:questionId/:answerId
      const res = await axiosInstance.delete(
              ...
              <div className="prose max-w-none mb-6">
                <div
                  ...
                  dangerouslySetInnerHTML={{
                    __html: renderMarkdown(question.questionbody),
                  }}
                />
              </div>
                  ...
                  <div className="flex-1 p-4 sm:p-6">
                    <div className="prose max-w-none mb-6">
                      <div
                        className="text-gray-800 leading-relaxed"
                        dangerouslySetInnerHTML={{
                          __html: renderMarkdown(ans.answerbody),
                        }}
                      />
                    </div>
                    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                      <div className="flex gap-2">
                        <Button
                          ...
                          className="text-gray-600 hover:text-gray-800"
                          onClick={() => {
                            navigator.clipboard.writeText(window.location.href + "#" + ans._id);
                            toast.success("Link copied!");
                          }}
                        >
                          <Share className="w-4 h-4 mr-1" />
